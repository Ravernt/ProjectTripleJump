...
public class HealthTests
{
    ...
    [UnityTest]
    public IEnumerator OnCollisionEnter2D_TouchSpike_TakeDamage()
    {
        player.transform.position = new(0, 2);
        health.MaxHealth = 3;
        health.CurrentHealth = 3;
        health.InvincibilityTime = 2f;

        GameObject spikes = (GameObject)GameObject.Instantiate(Resources.Load("Spikes"), Vector2.zero, Quaternion.identity);
        spikes.tag = "Hurtful";
    ...
    [UnityTest]
    public IEnumerator OnCollisionEnter2D_TouchDeadlySpike_Death()
    {
        player.transform.position = new(0, 2);
        health.MaxHealth = 3;
        health.CurrentHealth = 3;
        ...
        GameObject spikes = (GameObject)GameObject.Instantiate(Resources.Load("Spikes"), Vector2.zero, Quaternion.identity);
        spikes.tag = "InstaKill";
        yield return new WaitForSeconds(0.75f);

        Assert.IsTrue(health.Dead);
    }
}
